...
export default function Template3({ data = {}, accentColor = '#2D6A4F' }) {
    ...
    const Section = ({ title, children }) => (
        <div style={{ marginBottom: '18px' }}>
            <h3 style={{ fontSize: '12px', fontWeight: 600, color: accentColor, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '10px' }}>{resolveSectionTitle(title, data.atsStandardSectionNames)}</h3>
            {children}
        </div>
    );
    ...
    return (
        <div style={{
            ...
            <div style={{ marginBottom: '24px', borderBottom: `3px solid ${accentColor}`, paddingBottom: '16px', textAlign: 'center' }}>
                ...
                <h1 style={{ fontSize: '30px', fontWeight: 300, color: '#1C1C1C', letterSpacing: '-0.02em', margin: '0 0 8px' }}>{pi.name || 'Your Name'}</h1>
                <div style={{
                    display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center',
                    gap: '6px 16px', fontSize: '11px', color: '#6B6560', marginBottom: '6px'
                }}>
                    {pi.email && (
                ...
                <div style={{
                    display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center',
                    gap: '6px 14px', fontSize: '9.5pt', margin: 0
                }}>
                    {pi.linkedinUrl && (
            ...
            {(data.skillsText || data.skills?.length > 0) && (
                <Section title="Technical Skills">
                    <p style={{ fontSize: '11px', color: '#4a4a4a', lineHeight: 1.4, margin: 0 }}>
                    ...
                    {data.certifications.map((c, i) => (
                        <div key={i} style={{ marginBottom: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                {c.url && (
                                    <a href={c.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '9pt', color: accentColor, textDecoration: 'none' }}>Verify →</a>
                                )}
                            </div>
                        </div>
                    ))}
            ...
            {data.interests && data.interests.filter(i => i.trim() !== '').length > 0 && (
                <Section title="Interests">
                    <p style={{ fontSize: '11px', color: '#4a4a4a', margin: 0 }}>{data.interests.filter(i => i.trim() !== '').join(' · ')}</p>
                </Section>
            )}
