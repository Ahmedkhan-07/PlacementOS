export default function Template5({ data = {}, accentColor = '#2D6A4F' }) {
    ...
    const Section = ({ title, children }) => {
        ...
        return (
            <div style={{ marginBottom: isSidebarSec ? '20px' : '18px' }}>
                <h3 style={{
                    fontSize: isSidebarSec ? '10px' : '12px',
                    fontWeight: 700,
                    color: accentColor,
        ...
        <div style={{
            ...
            <div style={{ width: '30%', background: '#F5F3EF', padding: '32px 16px', borderRight: `3px solid ${accentColor}` }}>
                ...
                <div style={{ marginBottom: '20px' }}>
                    ...
                    {pi.email && <p style={{ fontSize: '10px', color: '#4a4a4a', wordBreak: 'break-all', margin: '0 0 4px' }}>{pi.email}</p>}
                    {pi.phone && <p style={{ fontSize: '10px', color: '#4a4a4a', margin: '0 0 4px' }}>{pi.phone}</p>}
                    {pi.location && <p style={{ fontSize: '10px', color: '#4a4a4a', margin: '0 0 6px' }}>{pi.location}</p>}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '10px', color: '#4a4a4a' }}>
                        {pi.linkedinUrl && (
                ...
                {data.languages?.length > 0 && (
                    <Section title="Languages">
                        <p style={{ fontSize: '10px', color: '#4a4a4a', margin: 0 }}>
                            {data.languages.join(', ')}
                        </p>
                    </Section>
                )}
            ...
            <div style={{ width: '70%', padding: '32px 24px' }}>
                <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#1C1C1C', marginBottom: '18px', textAlign: 'center' }}>{pi.name || 'Your Name'}</h1>
                    ...
                    <div style={{ marginBottom: '18px' }}>
                        ...
                        {data.experience.map((e, i) => (
                            <div key={i} style={{ marginBottom: '10px', paddingLeft: '12px', borderLeft: `2px solid ${accentColor}` }}>
                                ...
                                {e.description && <p style={{ fontSize: '11px', color: '#4a4a4a', margin: 0 }}>{e.description}</p>}
                            </div>
                        ))}
                    </div>
                )}
